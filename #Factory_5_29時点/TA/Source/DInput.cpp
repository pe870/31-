...
LPDIRECTINPUTDEVICE8	g_pPad;	//デバイスオブジェクト（コントローラー）
...
//エラーチェック
	if (hRlt != DI_OK)
	{
		...
		else
		{
			...
			hRlt = g_pPad->SetDataFormat(&c_dfDIJoystick2);
			if (hRlt != DI_OK)
			{
...
HRESULT DInput :: UpdateInputState()
{
	for (int i = 0; i < PadNumber; i++)
	{
		...
		//㊦.
		if (js.lRy > N_OFFSET){ AddInputState(enPkey_DR); }
		//㊤
		else if (js.lRy < -N_OFFSET) { AddInputState(enPkey_UR); }
		//㊨.
		if (js.lRx > N_OFFSET){ AddInputState(enPkey_RR); }
		//㊧.
		else if (js.lRx < -N_OFFSET) { AddInputState(enPkey_LR); }
